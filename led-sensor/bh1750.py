...
_BH1750_DEFAULT_ADDRESS = 0x23  # I2C address with ADDR pin low
_BH1750_ALT_ADDRESS = 0x5C  # I2C address with ADDR pin high

# Instructions
_BH1750_POWER_DOWN = 0x00  # Power down instruction
_BH1750_POWER_ON = 0x01  # Power on instruction
_BH1750_RESET = 0x07  # Reset instruction

# Bitfields
_BH1750_MODE_MASK = 0x30  # Mode mask bits
_BH1750_RES_MASK = 0x03  # Mode resolution mask bits

# Worst case conversion timing in ms
_BH1750_CONV_TIME_L = 24  # Worst case conversion timing low res
_BH1750_CONV_TIME_H = 180  # Worst case conversion timing high res
...
class BH1750:  # pylint:disable=too-many-instance-attributes
    ...
    **Quickstart: Importing and using the BH1750**
    ...
    def __init__(self, pi: pigpio, address: int = _BH1750_DEFAULT_ADDRESS) -> None:
        self.pi = pi
        self.i2c_device = pi.i2c_open(1, address)
    ...
    def _raw_reading(self) -> int:
        ...
        (b,d) = self.pi.i2c_read_device(self.i2c_device, 2)
        self._buffer = d
    ...
    def _write(self, cmd_byte: int) -> None:
        ...
        self.pi.i2c_write_byte(self.i2c_device, self._buffer[0])
